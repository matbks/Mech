	...
	function (BaseController, JSONModel) {
	  ...
	  return BaseController.extend("sap.ui.demo.masterdetail.controller.Order", {
		...
		onBeforeRendering: function () {
		  this.byId("pes").setModel(this.jModel);
		},
		onNavBack: function () {
		  history.go(-2);
		},
		_onObjectMatched: function (oEvent) {
		  var sAnimalId = oEvent.getParameter("arguments").Plateid;
		  this.getModel()
			.metadataLoaded()
			.then(
			  function () {
				var sAnimalPath = this.getModel().createKey("VehiclesSet", {
				  Id: sAnimalId,
				});
				this._bindView("/" + sVehiclePath);
			  }.bind(this)
			);
		},
		...
		addRow: function (oArg) {
		  this._data.Products.push({ Description: "", Date: "", Quantity:"", Unit:"", Price:""});
